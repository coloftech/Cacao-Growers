            ...
            <form class="form" id="frmPersonalInfo">
              ...
              <div class="form-group width-full">
                ...
                <div class="choices">
                  ...
                  <input type="text" name="fname" id="fname" class="form-control" placeholder="First name"  style="text-transform: capitalize;" tabindex="1"  value="<?=isset($basicinfo[0]->fname) ? $basicinfo[0]->fname: '';?>" required> <input type="text" name="lname" id="lname" class="form-control" placeholder="Last name"  style="text-transform: capitalize;" tabindex="2" value="<?=isset($basicinfo[0]->lname) ? $basicinfo[0]->lname: '';?>" required>
                  </div>
                </div>
              ...
              <div class="form-group width-full">
                ...
                <div class="choices">
                  <div class="width-full">
                            ...
                            <input type="date" class="form-control" name="date_of_survey" value="<?=isset($basicinfo[0])?date('Y-m-d',strtotime($basicinfo[0]->date_of_survey)):date('Y-m-d')?>" max="<?=date('Y-m-d')?>" tabindex="6">
                  </div>
                </div>
              </div>
              <div class="form-group width-full">
                ...
                <div class="choices">
                  <div class="width-full">
                            ...
                            <input type="text" name="address" id="address" class="form-control" placeholder="Address" tabindex="7" value="<?=isset($basicinfo[0]->address) ? $basicinfo[0]->address: '';?>" required>
                  </div>
                </div>
              </div>
              <div class="form-group width-full">
                ...
                <div class="choices">
                  ...
                  <input oninput="this.value=this.value.slice(0,this.maxLength)" type="number" name="age" id="age" min="0" max='120' class="form-control" maxlength="3" placeholder="Age" tabindex="8" value="<?=isset($basicinfo[0]->age) ? $basicinfo[0]->age: 0;?>" required>
                  </div>
                </div>
              </div>
              <div class="form-group width-full">
              ...
              <div class="form-group width-full">
                ...
                <div class="choices">
                  <div class="width-full">
                    <input type="text" class="form-control" id="familysize" name="familysize"  tabindex="12" value="<?=isset($basicinfo[0]->familysize) ? $basicinfo[0]->familysize: 0;?>" required>                    
                  </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <div id="cityid" class="hidden"><?=$basicinfo[0]->city?></div>
        </div>
        ...
        <div class="form-group">
          <label>Barangay</label><select name="barangay" id="barangay" class="form-control"></select><div id="brgyid" class="hidden"><?=$basicinfo[0]->barangay?></div>
        </div>
        <div class="form-group">
...
<div class="hidden">
  <div id="vinfo"><?=isset($basicinfo[0]) ? json_encode($basicinfo[0]) : false; ?></div>
</div>
...
var brgyArr = [];
  ...
  var jqxhr = $.getJSON( url, function(records, i) {
      ...
      brgyArr.push(record);
       ...
       $('#barangay').append($("<option></option>").attr("value",i.id).attr('selected',true).text(i.brgyDesc));
        }else{
  ...
  var jqxhr = $.getJSON( url, function(records, i) {
      ...
      if (id == i.id) {
       ...
       $('#city').append($("<option></option>").attr("value",i.id).attr('selected',true).data('citymunCode',i.citymunCode).text(citymunDesc));
      }else{
...
<script type="text/javascript">
  $(function(){
    var vinfo = $('#vinfo').html();
    if (vinfo) {
      vinfo = JSON.parse(vinfo)      
      checkedRadioInfo(vinfo)

    }
  })

  var checkedRadioInfo = function(data){
    $('#frmPersonalInfo input:radio').removeAttr('checked');
    var radio = $('#frmPersonalInfo input:radio');
    $.each(radio,function(){

      var val = $(this).val();
      var parent = $(this).attr('name');
      
      if($.trim(data.civilstatus) == $.trim(val) &&  'civilstatus' == parent ){
        $(this).prop('checked',true);
      }
      if($.trim(data.education) == $.trim(val) &&  'education' == parent ){
        $(this).prop('checked',true);
      }
      if($.trim(data.annualincome) == $.trim(val) &&  'annualincome' == parent ){
        $(this).prop('checked',true);
      }
      if($.trim(data.civilstatus) == $.trim(val) &&  'civilstatus' == parent ){
        $(this).prop('checked',true);
