...
var pest = <?php echo json_encode(explode(',', isset($pestsdiseases[0]->pest)?$pestsdiseases[0]->pest:'')); ?>;
var disease = <?php echo json_encode(explode(',', isset($pestsdiseases[0]->disease) ? $pestsdiseases[0]->disease : '')); ?>;
var disease_control = <?php echo json_encode(explode(',', isset($pestsdiseases[0]->disease_control) ? $pestsdiseases[0]->disease_control : '')); ?>;
var pest_control_management = <?php echo json_encode(explode(',', isset($pestsdiseases[0]->pest_control_management) ? $pestsdiseases[0]->pest_control_management : '')); ?>;

  $('#frmpest input:checkbox').each(function() {
    

      var input = $(this);
      var val = $(this).parent().text();
        if (pest.length > 0) {
          pest.forEach(function(item) {  
    ...
    if (pest_control_management.length > 0) {
          pest_control_management.forEach(function(item) {  
        ...
        if (disease_control.length > 0) {
          disease_control.forEach(function(item) {  
