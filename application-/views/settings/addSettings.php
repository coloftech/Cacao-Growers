					...
					<?php if (is_array($homeSettings)): ?>
						<?php foreach ($homeSettings as $key): ?>
							<tr data-id='<?=$key->settings_id?>' id="<?=$key->settings_id?>">
								...
								<td class="setting_value"><?=$this->messages->limit(html_entity_decode($key->settings_value),70)?><div class="hidden"><?=html_entity_decode($key->settings_value)?></div></td>
...
<script type="text/javascript">
	...
	function savedata(id,content) {
		...
		var data = {
			'settings_value':content,
			'settings_id':id,
			'settings_name':$('input[name="settings_name"]').val(),
			'is_active':$('select[name="settings_status"]').val()
		};
		$.ajax({
